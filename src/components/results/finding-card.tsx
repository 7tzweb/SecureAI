  ...
  return (
    <div className="rounded-[1.2rem] border border-white/70 bg-white/70 p-4 shadow-[0_8px_24px_rgba(148,163,184,0.12)]">
      ...
      <div className="mt-3 space-y-2 text-xs leading-6 text-slate-500">
        {location.path ? (
          <p>
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">Path</span>{" "}
            {location.path}
          </p>
        ) : null}
        {location.selector ? (
          <p>
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">Selector</span>{" "}
            {location.selector}
          </p>
        ) : null}
        {location.url ? (
          <p className="break-all">
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">URL</span>{" "}
            {location.url}
          </p>
        ) : null}
        {location.attribute ? (
          <p>
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">Attribute</span>{" "}
            {location.attribute}
          </p>
        ) : null}
        {location.value ? (
          <p className="break-all">
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">Value</span>{" "}
            {location.value}
          </p>
        ) : null}
        {location.context ? (
          <p>
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">Context</span>{" "}
            {location.context}
          </p>
        ) : null}
        {location.note ? (
          <p>
            <span className="font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">Note</span>{" "}
            {location.note}
          </p>
        ) : null}
      </div>
...
export function FindingCard({
  ...
  return (
    <article className="glass-panel flex items-start gap-6 rounded-[2rem] bg-white/60 p-6 transition-all hover:shadow-[0_12px_40px_rgba(0,0,0,0.05)]">
      <div
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-2 flex flex-wrap items-center gap-3">
          <h3 className="text-lg font-semibold tracking-[-0.01em] text-slate-900">{finding.title}</h3>
          <span
            className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] ${getStatusStyles(status)}`}
          >
            {status}
          </span>
          {(status === "warning" || status === "fail") ? (
            <span
              className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] ${getSeverityStyles(finding.severity)}`}
            >
              {finding.severity}
            </span>
          ) : null}
          ...
          <div className="mt-4 space-y-3">
            <div className="grid gap-3 lg:grid-cols-2">
              <div className="rounded-[1.4rem] bg-[var(--surface-muted)] p-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--ink-soft)]">
                  Why it matters
                </p>
      ...
      <div className="shrink-0">
        {finding.locked ? (
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-white/50 px-5 py-2.5 text-sm font-semibold text-[var(--primary)] shadow-sm transition-all hover:bg-white"
          >
